...
-spec extract_context_fragments(swag_server:request_context(), woody_context:ctx()) ->
    wapi_bouncer_context:fragments() | undefined.
...
extract_context_fragments_by(claim, {_, _, Claims, _}, _) ->
    ...
    case get_claim(Claims) of
        {ok, ClaimFragment} ->
            {Acc, External} = wapi_bouncer_context:new(),
...
-spec build_auth_context_fragments(
    wapi_auth:auth_method(),
    uac:context(wapi_auth:metadata()),
    woody_context:ctx()
...
build_auth_context_fragments(user_session_token, AuthCtx = {_, _, _, Metadata}, WoodyCtx) ->
    ...
    {Acc0, External} = wapi_bouncer_context:new(),
...
-spec get_claim(claims()) -> {ok, wapi_bouncer_context:fragment()} | {error, {unsupported, claim()}} | undefined.
get_claim(Claims) ->
...
-spec decode_claim(claim()) ->
    {ok, wapi_bouncer_context:fragment()} | {error, {unsupported, claim()} | {malformed, binary()}}.
decode_claim(#{
...
-spec set_claim(wapi_bouncer_context:fragment(), claims()) -> claims().
...
-spec encode_claim(wapi_bouncer_context:fragment()) -> claim().
encode_claim(
    {encoded_fragment, #bctx_ContextFragment{
        type = v1_thrift_binary,
        content = Content
