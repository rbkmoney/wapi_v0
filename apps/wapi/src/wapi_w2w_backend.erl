...
-spec get_transfer(req_data(), handler_context()) -> {ok, response_data(), id()} | {error, GetError} when
    GetError ::
    ...
    case service_call(Request, HandlerContext) of
        {ok, TransferThrift} ->
            case wapi_access_backend:check_resource(w2w_transfer, TransferThrift, HandlerContext) of
                ok ->
                    Owner = wapi_access_backend:get_resource_owner(transfer, TransferThrift),
